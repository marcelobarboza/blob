...
date_t time_travel(date_t d, int ndays) {
  return from_ordinal(to_ordinal(d) + ndays);
}

int ordering(date_t d1, date_t d2) {
...
void bubble_sort(const int h, date_t *const H) {
  int i;
  int j;
  date_t d_tmp;
  for (i = 0; i < h - 1; i++)
...
bool found_in(const int h, const date_t *const H, const date_t d) {
  bool found = false;
  int lower_bound = 0;
  int upper_bound = h - 1;
  int mean;
